...
def simulation(triangle,num_particles, ts, filename=None):
	...
	S_4s[0],outcomes4[0],lambdas4 = rho_num.S_G(
		[x_avgs[0],y_avgs[0],x2_avgs[0],y2_avgs[0]],np.array([-.1,.1,.1,.1,.1]))
	...
	for i in range(1,num_ts):
		...
		x_avgs[i],y_avgs[i],x2_avgs[i],y2_avgs[i] = compute_avgs(x_list,y_list)
...
def main():
	...
	plot_results(ts,x_avgs,y_avgs,x2_avgs,y2_avgs,S_1s,S_2s,S_3s,S_4s,S_Ss)
